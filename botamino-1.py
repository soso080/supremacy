...
def joinamino(args):
    ...
    if not isJoined:
        ...
        if isJoined:
            ...
            client.get_community(comId).ask_amino_staff(
                f"Hello! I am a bot and i can do a lot of stuff!\nI've been invited here by {auth}.\nIf you need help, you can do !help.\nEnjoy^^")
...
def ask(args):
    ...
    args.subClient.ask_all_members(
...
def lock_command(args):
    if not args.message or args.message in args.subClient.locked_command or args.message not in client.commands_list() or args.message in (
    "lock", "unlock"):
        return
    try:
...
def everyone(args):
    ...
    args.subClient.send_message(chatId=args.chatId, message=f"[iu]@everyone‎‏‎‏‬‭‎‏‎‏‬‭ {args.message}",
...
def gif(args):
    ...
    response = requests.get(
        'http://api.giphy.com/v1/gifs/search?q=' + search + '&api_key=7G8jLZHM52O5YLJ0fPcBOawMvew5a1e1')
    # print(response.text)
    ...
    if not args.message:
        ...
        try:
            ...
            args.subClient.send_message(args.chatId,
                                        f'🎲 -{sum(numbers)},[ {" ".join(map(str, numbers))}](1-{max_num})- 🎲')
        except Exception as e:
            print_exception(e)
...
@client.on_message()
def on_message(data):
    [data.subClient.delete_message(data.chatId, data.messageId, reason=f"{data.message}", asStaff=True) for elem in
     link_list if elem in data.message]
...
def on_chat_invite(data):
    ...
    subClient.send_message(args.chatId,
...
[BC]❀❀✿✿❀❀✿✿❀❀✿✿❀❀
                        ...
                        ┗ღ❦ღ━━━━━━━━┛
                                            ヽ(‧ω‧`)ﾉ
                                                 |    |
                                                 UU
''', embedTitle=data.author, embedLink=f"ndc://user-profile/{data.authorId}",
                                embedImage=upload(data.info.message.author.icon))
